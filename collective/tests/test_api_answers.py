...
class AnswerListViewTests(AuthTestCase):
    def test_reverse(self):
        self.assertEqual(reverse('answers', args=['jla']), '/api/collective/jla/answers/')
    ...
    def test_get_answer_list(self):
        ...
        q_1 = QuestionnaireItem.objects.create(collective=collective, name='q1', title='Question 1', order=1,
                                               creator=creator)
        q_2 = QuestionnaireItem.objects.create(collective=collective, name='q2', title='Question 2', order=2,
                                               creator=creator)
        a_1 = Answer.objects.create(question=q_1, user=user, vote=1, comment='Of course')
        a_2 = Answer.objects.create(question=q_2, user=user, vote=-1, comment='Definitely not')
