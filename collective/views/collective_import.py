...
def parse_imported_data(data):
    ...
    if Collective.objects.filter(name=data["name"]).count() > 0:
        logger.warning(
            "Collective with name {} already exists. Aborting import.".format(
                data["name"]
            )
        )
        return None
    ...
    for question in data["questions"]:
        try:
            ...
            logger.warning(
                "Question creator {} does not exist. Creating.".format(creator_name)
            )
        ...
        for answer in question["answers"]:
            try:
                ...
                logger.warning(
                    "Answer user {} does not exist. Creating.".format(answer["user"])
                )
